document.addEventListener('DOMContentLoaded', function () {
    ...
    function populateLanguagesSelect(routeId) {
        ...
        fetch(apiUrl)
            ...
            .then(data => {
                ...
                // Добавление варианта "не выбрано"
                const defaultOption = document.createElement('option');
                defaultOption.value = ''; // Пустое значение для "не выбрано"
    ...
    document.getElementById('calculateCost').addEventListener('click', function () {
        ...
        let studentDiscountMultiplier = 1;
        let interpreterSupportMultiplier = 1;

        if (useSeniorDiscount) {
            studentDiscountMultiplier = 0.85; // Скидка для пенсионеров: 15%
        }

        if (useInterpreterSupport) {
            interpreterSupportMultiplier = 1.5;
        }
        const totalPrice = calculatePrice(
            guidePricePerHour * studentDiscountMultiplier * interpreterSupportMultiplier,
            hoursNumber,
            isThisDayOff,
            isItMorning,
            isItEvening,
        ...
        document.getElementById('sendOrder').addEventListener('click', function () {
            fetch('http://exam-2023-1-api.std-900.ist.mospolytech.ru/api/orders?api_key=3990d76f-4908-438c-a2e9-90a0a642eb96', {
                method: 'POST',
                headers: {
